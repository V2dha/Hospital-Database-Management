...
import pymysql
mydb = pymysql.connect(host='localhost', user='root', password='toor')
cur = mydb.cursor()
...
def Viewregist():
    ...
    image1_label = Label(view_r, image=image1, height=1000, width=2000)
    ...
    def submit():
        ...
        path1 = r'1.jpg'
        image1 = ImageTk.PhotoImage(Image.open(path1))
        image1_label = Label(view, image=image1, height=1000, width=2000)
        image1_label.image = image1
        image1_label.place(x=0, y=0)
        ...
        cur.execute('create table if not exists project.hospital(name varchar(255), last varchar(255), age varchar(255), gender varchar(255), phone varchar(255), appt varchar(255))')
        ...
        Button(view, text='Back', relief=RAISED, bg='white', font=('ariel', 20), command=view.destroy).place(x=680, y=500)

        view.mainloop()

    # ------------------Button---------------------------
    Button(view_r, text='Back', relief=RAISED, bg='white', font=('ariel', 20), command=view_r.destroy).place(x=600, y=300)
